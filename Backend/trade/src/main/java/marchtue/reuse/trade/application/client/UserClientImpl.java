...
public class UserClientImpl implements UserClient {
  ...
  @Override
  public List<InternalBuyerInfoResponse> getBuyerInfoList(List<UUID> userIds) {
    String url = "http://user-service:19091/internal/users/buyer-infos";
    HttpEntity<List<UUID>> request = new HttpEntity<>(userIds);
    ResponseEntity<InternalBuyerInfoResponse[]> response = restTemplate.postForEntity(url, request,
        InternalBuyerInfoResponse[].class);
    return Arrays.asList(Objects.requireNonNull(response.getBody()));
  }
